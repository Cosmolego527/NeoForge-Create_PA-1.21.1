...
import com.cosmolego527.create_pp.ModGuiTextures;
...
public class VoidFunctionScreen extends AbstractSimiContainerScreen<VoidFunctionMenu> implements ScreenWithStencils {
    ...
    @Override
    protected void init() {
        ModGuiTextures bg = ModGuiTextures.SCHEDULE_TEST;
    ...
    protected void renderSchedule(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i = 0; i <= entries.size(); i++) {
            ...
            if (schedule.savedProgress == i && !schedule.entries.isEmpty()) {
                ...
                (expectedY == actualY ? ModGuiTextures.SCHEDULE_POINTER_TEST : ModGuiTextures.SCHEDULE_POINTER_OFFSCREEN_TEST)
            ...
            if (i == 0 || entries.size() == 0)
                UIRenderHelper.drawStretched(graphics, leftPos + 33, topPos + 16, 3, 10, -100,
                        ModGuiTextures.SCHEDULE_STRIP_LIGHT_TEST);

            if (i == entries.size()) {
                if (i > 0)
                    yOffset += 9;
                ModGuiTextures.SCHEDULE_STRIP_END_TEST.render(graphics, leftPos + 29, topPos + yOffset);
                ModGuiTextures.SCHEDULE_CARD_NEW_TEST.render(graphics, leftPos + 43, topPos + yOffset);
                matrixStack.popPose();
                endStencil();
                break;
            }
            ...
            if (i + 1 < entries.size()) {
                ModGuiTextures.SCHEDULE_STRIP_DOTTED_TEST.render(graphics, leftPos + 29, topPos + yOffset - 3);
                yOffset += 10;
            }
            ...
            if (isConditionAreaScrollable(scheduleEntry)) {
                ...
                if (!Mth.equal(chaseTarget, 0))
                    ModGuiTextures.SCHEDULE_SCROLL_LEFT_TEST.render(graphics, leftPos + 40, topPos + cardY + center);
                if (!Mth.equal(chaseTarget, scheduleEntry.conditions.size() - 1))
                    ModGuiTextures.SCHEDULE_SCROLL_RIGHT_TEST.render(graphics, leftPos + 203, topPos + cardY + center);
                matrixStack.popPose();
                endStencil();
            }
        }
    ...
    public int renderScheduleEntry(GuiGraphics graphics, ScheduleEntry entry, int yOffset, int mouseX, int mouseY,
        ...
        ModGuiTextures light = ModGuiTextures.SCHEDULE_CARD_LIGHT_TEST;
        ModGuiTextures medium = ModGuiTextures.SCHEDULE_CARD_MEDIUM_TEST;
        ModGuiTextures dark = ModGuiTextures.SCHEDULE_CARD_DARK_TEST;
        ...
        ModGuiTextures.SCHEDULE_CARD_REMOVE_TEST.render(graphics, cardWidth - 14, 2);
        ModGuiTextures.SCHEDULE_CARD_DUPLICATE_TEST.render(graphics, cardWidth - 14, cardHeight - 14);

        int i = schedule.entries.indexOf(entry);
        if (i > 0)
            ModGuiTextures.SCHEDULE_CARD_MOVE_UP_TEST.render(graphics, cardWidth, cardHeader - 14);
        if (i < schedule.entries.size() - 1)
            ModGuiTextures.SCHEDULE_CARD_MOVE_DOWN_TEST.render(graphics, cardWidth, cardHeader);

        UIRenderHelper.drawStretched(graphics, 8, 0, 3, cardHeight + 10, zLevel, ModGuiTextures.SCHEDULE_STRIP_LIGHT_TEST);
        (supportsConditions ? ModGuiTextures.SCHEDULE_CARD_MOVE_UP_TEST : ModGuiTextures.SCHEDULE_STRIP_ACTION_TEST)
                .render(graphics, 4, 6);

        if (supportsConditions)
            ModGuiTextures.SCHEDULE_STRIP_WAIT_TEST.render(graphics, 4, 28);
    ...
    public void renderScheduleConditions(GuiGraphics graphics, ScheduleEntry entry, int yOffset, int mouseX, int mouseY,
        ...
        for (List<ScheduleWaitCondition> list : entry.conditions) {
            ...
            ModGuiTextures.SCHEDULE_CONDITION_APPEND_TEST.render(graphics, xOffset + (maxWidth - 10) / 2,
                    29 + list.size() * 18);
            xOffset += maxWidth + 10;
        }

        ModGuiTextures.SCHEDULE_CONDITION_NEW_TEST.render(graphics, xOffset - 3, 29);
        matrixStack.popPose();
    ...
    protected int renderInput(GuiGraphics graphics, Pair<ItemStack, Component> pair, int x, int y, boolean clean,
        ...
        ModGuiTextures left =
                clean ? ModGuiTextures.SCHEDULE_CONDITION_LEFT_CLEAN_TEST : ModGuiTextures.SCHEDULE_CONDITION_LEFT_TEST;
        ModGuiTextures middle = ModGuiTextures.SCHEDULE_CONDITION_MIDDLE_TEST;
        ModGuiTextures item = ModGuiTextures.SCHEDULE_CONDITION_ITEM_TEST;
        ModGuiTextures right = ModGuiTextures.SCHEDULE_CONDITION_RIGHT_TEST;
    ...
    private Component clickToEdit = CreatePPLang.translateDirect("gui.schedule.lmb_edit")
            .withStyle(ChatFormatting.DARK_GRAY, ChatFormatting.ITALIC);
    private Component rClickToDelete = CreatePPLang.translateDirect("gui.schedule.rmb_remove")
            .withStyle(ChatFormatting.DARK_GRAY, ChatFormatting.ITALIC);
    ...
    public boolean action(@Nullable GuiGraphics graphics, double mouseX, double mouseY, int click) {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            if (x > 180 && x <= 192) {
                if (y > 0 && y <= 14) {
                    renderActionTooltip(graphics,
                            ImmutableList.of(CreatePPLang.translateDirect("gui.schedule.remove_entry")), mx, my);
                    if (click == 0) {
                ...
                if (y > cardHeight - 14) {
                    renderActionTooltip(graphics,
                            ImmutableList.of(CreatePPLang.translateDirect("gui.schedule.duplicate")), mx, my);
                    if (click == 0) {
            ...
            if (x > 194) {
                if (y > 7 && y <= 20 && i > 0) {
                    renderActionTooltip(graphics, ImmutableList.of(CreatePPLang.translateDirect("gui.schedule.move_up")),
                            mx, my);
                    if (click == 0) {
                        entries.remove(entry);
                ...
                if (y > 20 && y <= 33 && i < entries.size() - 1) {
                    renderActionTooltip(graphics,
                            ImmutableList.of(CreatePPLang.translateDirect("gui.schedule.move_down")), mx, my);
                    if (click == 0) {
                        entries.remove(entry);
                        entries.add(i + 1, entry);
                        init();
            ...
            for (int j = 0; j < columns.size(); j++) {
                ...
                if (row < conditions.size() && row >= 0) {
                    ...
                    List<Component> components = new ArrayList<>();
                    components.add(CreatePPLang.translateDirect("schedule.condition_type")
                ...
                if (y > 18 * conditions.size() && y <= 18 * conditions.size() + 10 && x >= w / 2 - 5 && x < w / 2 + 5) {
                    renderActionTooltip(graphics,
                            ImmutableList.of(CreatePPLang.translateDirect("gui.schedule.add_condition")), mx, my);
                    if (click == 0)
            ...
            renderActionTooltip(graphics,
                    ImmutableList.of(CreatePPLang.translateDirect("gui.schedule.alternative_condition")), mx, my);
            if (click == 0)
                startEditing(new ScheduledDelay(), confirmed -> {
                    if (!confirmed)
                        return;
        ...
        renderActionTooltip(graphics, ImmutableList.of(CreatePPLang.translateDirect("gui.schedule.add_entry")), mx, my);
        if (click == 0)
    ...
    protected void renderBg(GuiGraphics graphics, float pPartialTick, int pMouseX, int pMouseY) {
        ...
        formattedcharsequence = editingCondition == null ? CreatePPLang.translateDirect("schedule.instruction.editor")
                .getVisualOrderText()
                : CreatePPLang.translateDirect("schedule.condition.editor")
                .getVisualOrderText();
        graphics.drawString(font, formattedcharsequence, (float) (center - font.width(formattedcharsequence) / 2),
